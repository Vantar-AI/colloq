...
**Current evidence:** the request/token/cancel graph now compiles once into a verified, identified Eve Plan whose shared endpoint graphs create lightweight sessions over memory, TCP, authenticated QUIC, and mutually authenticated Iroh. Reference and compact paths reconstruct the same semantic traces across all four transports. Network peers exchange a strict version/conversation/plan/role/encoding preface and reject mismatches before frame zero; Iroh additionally binds persistent endpoint identities and a TLS exporter from the concrete connection. An Automerge draft store supports optimistic scalar graph patches, convergent ordered sync, and a conflict-rejecting validation/promotion gate. A Miren adapter emits a pinned container and manifest whose expected Eve identities are enforced at server startup. Compact encoding improves the isolated transition by 1.32× and the full warm workload by 1.11×, though it remains 1.51× the hand-written baseline. Deterministic faults preserve asymmetric timeout and uncertainty observations. Actual multi-node execution, role authorization and identity lifecycle, replay-resistant freshness, canonical identity, typed composite graph transactions, recovery semantics, and representative AI benchmarks remain open.
